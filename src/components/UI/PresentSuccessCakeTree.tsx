...
export default function PresentSuccessCakeTree({ cakeList }: { cakeList: CakeTreeDataType[] }) {
    ...
    <div className="flex flex-col items-center w-full">
      <div className="relative w-375 h-screen mt-40">
        ...
        {Array.from({ length: numberOfRows }).map((_, rowIndex) => {
          const cakesInRow = restCake.slice(rowIndex * 3, rowIndex * 3 + 3);
          const rowKey = cakesInRow.map((c) => c.cakeId).join('-');
