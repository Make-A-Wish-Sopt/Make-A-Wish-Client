...
import { getWishes, deleteWishes } from '@/api/wishes';
...
function DeleteWishButton({ selectedItems }: { selectedItems: number[] }) {
    ...
    >
      ...
      >
        <Modal.ButtonWrapper className="flex justify-between gap-10 w-full">
          ...
          <Button bgColor="dark_green" fontColor="white" onClick={handleDeleteWishes}>
            예
          </Button>
        </Modal.ButtonWrapper>
      </ConfirmModalContent>
...
WishHistoryBox.displayName = 'WishHistoryBox';

function WishHistoryList({
  wishesHistory,
  selectItem,
  cancelItem,
}: {
  ...
  return (
    <div className="flex flex-col items-center">
      <Image
        ...
        width={293}
      />
      <Button onClick={() => handleRouter('/wishes')}>생일잔치 링크 생성하기</Button>
    </div>
  );
}
