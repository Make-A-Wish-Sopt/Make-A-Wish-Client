...
export function MypageMenuContainer({ children }: PropsWithChildren) {
...
export function ServiceGuideMenu() {
...
export function CloseWishMenu({ disabled }: { disabled?: boolean }) {
        ...
        <ConfirmModalContent contentTitle={`진행 중인 생일잔치를\n중단하시겠어요?`}>
          <Modal.ButtonWrapper className="flex justify-between gap-10 w-full">
            <Button
              ...
              onClick={() => closeModal('closeWish')}
            >
              취소
            </Button>
            <Button type="button" bgColor="dark_green" fontColor="white" onClick={handleCloseWish}>
              중단하기
            </Button>
          </Modal.ButtonWrapper>
        </ConfirmModalContent>
        ...
        <LogoutModal
          ...
          Trigger={
            <li className={authButtonStyle}>
              <button type="button" onClick={() => openLogoutModal('logout')} className="underline">
                로그아웃
              </button>
            </li>
          }
          ...
          <ConfirmModalContent contentTitle={`서비스에서\n로그아웃 하시겠어요?`}>
            <LogoutModal.ButtonWrapper className="flex justify-between gap-10 w-full">
              <Button
                ...
                bgColor="white"
                fontColor="dark_green"
                onClick={() => closeLogoutModal('logout')}
              >
                아니요
              </Button>
              <Button type="button" bgColor="dark_green" fontColor="white" onClick={handleLogout}>
                예
              </Button>
