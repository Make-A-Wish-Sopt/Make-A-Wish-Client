...
import SelectWantsGiftOption from './link/selectWantsGiftOption';
...
export default function WishesLinkInputForm({ wishTitle }: { wishTitle?: string }) {
  return (
    <WishesLinkInputs wishTitle={wishTitle}>
      <InputForm title="생일 선물도 받고 싶어요!">
        <SelectWantsGiftOption>{/* <DropDownContent /> */}</SelectWantsGiftOption>
      </InputForm>
    </WishesLinkInputs>
  );
}
...
export function WishesPeriod({ disabled }: { disabled?: boolean }) {
...
export function ImageToBeShownToGiver() {
  const { imageUrl, uploadImageFile } = useUploadImage();
