...
export default function WishesAccountInputForm({
  ...
  const { toggleState: accountNoticeAgree, changeToggleState: changeAccountNoticeAgree } =
    useToggle();
...
function SelectBank({
  ...
  const { toggleState: modalState, handleToggle: handleChangeModalState } = useToggle();

  return (
    <>
      <DropDwonBox isOpen={false} handleToggle={handleChangeModalState} bgColor="dark_green">
        <input
          {...register}
          placeholder="은행 선택"
      ...
      {modalState && (
        <Modal isOpen={modalState} handleToggle={handleChangeModalState}>
          <BankModal changeBank={changeBank} handleToggle={handleChangeModalState} />
        </Modal>
      )}
    </>
  );
...
function AccountFormNotice({
  ...
  const { toggleState: checkedState, changeToggleState: changeCheckedState } = useToggle();
